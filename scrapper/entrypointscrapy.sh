...
(crontab -l 2>/dev/null; echo "*/45 * * * * cd /app && /usr/local/bin/python hist_films_actu.py > /var/log/hist_films_actu.log 2>&1") | crontab -
(crontab -l 2>/dev/null; echo "*/45 * * * * cd /app && /usr/local/bin/python predict_films_actu.py > /var/log/predict_films_actu.log 2>&1") | crontab -
